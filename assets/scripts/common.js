...
$(function () {
	...
	$('.b-bank__name').click(function() {
		var text=$(this).text();
		if(!accname){
			accname=$(this).find('span').text();
		}
		if($(window).width()>=740) {
			$(this).attr('data-text',text).html('<input type="text" class="new_name" style="width:250px;" value="'+accname+'"/>').find('input').focus();
			//console.log(accname);
		}
		else {
	...
	$('body').on('blur', '.new_name', function(e) {
		...
		//console.log($(this).parent().attr('snum'));
		var accnum = $(this).parent().attr('snum');
